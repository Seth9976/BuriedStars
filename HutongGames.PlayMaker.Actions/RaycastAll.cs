...
public class RaycastAll : FsmStateAction
{
	...
	private void DoRaycast()
	{
		...
		Vector3 origin = ((!(ownerDefaultTarget != null)) ? fromPosition.Value : ownerDefaultTarget.transform.position);
		...
		{
			vector = ownerDefaultTarget.transform.TransformDirection(direction.Value);
		}
		RaycastAllHitInfo = Physics.RaycastAll(origin, vector, num, ActionHelpers.LayerArrayToLayerMask(layerMask, invertMask.Value));
