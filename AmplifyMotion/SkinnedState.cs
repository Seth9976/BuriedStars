...
internal class SkinnedState : MotionState
{
	...
	private bool InitializeGPUSkinDeform()
	{
		...
		try
		{
			...
			m_gpuBones = new Texture2D(m_gpuBoneTexWidth, m_gpuBoneTexHeight, TextureFormat.RGBAFloat, mipmap: false, linear: true);
			...
			m_gpuBoneIndices = new Texture2D(m_gpuVertexTexWidth, m_gpuVertexTexHeight, textureFormat, mipmap: false, linear: true);
			...
			for (int i = 0; i < m_vertexCount; i++)
			{
				...
				reference = new Vector4(boneWeight.boneIndex0, boneWeight.boneIndex1, boneWeight.boneIndex2, boneWeight.boneIndex3);
			}
			...
			{
				m_gpuBaseVertices[j] = new Texture2D(m_gpuVertexTexWidth, m_gpuVertexTexHeight, TextureFormat.RGBAFloat, mipmap: false, linear: true);
			...
			for (int l = 0; l < m_vertexCount; l++)
			{
				...
				{
					ref Color reference2 = ref list[m][num6];
					reference2 = m_baseVertices[l * m_weightCount + m];
				}
			}
			...
			for (int num8 = 0; num8 < m_vertexCount; num8++)
			{
				...
				ref Vector2 reference3 = ref array2[num8];
				reference3 = new Vector2(x, y);
			}
	...
	private void UpdateBonesGPU()
	{
		for (int i = 0; i < m_boneCount; i++)
		{
			...
			{
				ref Color reference = ref m_gpuBoneData[j * m_gpuBoneTexWidth + i];
				reference = m_bones[i].GetRow(j);
			}
		}
	...
	private void UpdateBones()
	{
		for (int i = 0; i < m_boneCount; i++)
		{
			ref Matrix3x4 reference = ref m_bones[i];
			reference = ((!(m_boneTransforms[i] != null)) ? Matrix4x4.identity : m_boneTransforms[i].localToWorldMatrix);
		}
	...
	private void AsyncUpdateVertices(bool starting)
	{
		...
		{
			ref Matrix3x4 reference = ref m_bones[i];
			reference = m_worldToLocalMatrix * m_bones[i];
		}
		if (m_weightCount == 1)
		{
			UpdateVerticesBone1();
	...
	private void InitializeBone1(Vector4[] baseVertices, int[] boneIndices)
	{
		...
		for (int i = 0; i < m_vertexCount; i++)
		{
			...
			ref Vector4 reference = ref baseVertices[num];
	...
	private void InitializeBone2(Vector4[] baseVertices, int[] boneIndices, float[] boneWeights)
	{
		...
		for (int i = 0; i < m_vertexCount; i++)
		{
			...
			ref Vector4 reference = ref baseVertices[num];
			reference = new Vector4(vector.x, vector.y, vector.z, weight);
			ref Vector4 reference2 = ref baseVertices[num2];
			reference2 = new Vector4(vector2.x, vector2.y, vector2.z, weight2);
		}
	}
	...
	private void InitializeBone4(Vector4[] baseVertices, int[] boneIndices, float[] boneWeights)
	{
		...
		for (int i = 0; i < m_vertexCount; i++)
		{
			...
			ref Vector4 reference = ref baseVertices[num];
			reference = new Vector4(vector.x, vector.y, vector.z, num9);
			ref Vector4 reference2 = ref baseVertices[num2];
			reference2 = new Vector4(vector2.x, vector2.y, vector2.z, num10);
			ref Vector4 reference3 = ref baseVertices[num3];
			reference3 = new Vector4(vector3.x, vector3.y, vector3.z, num11);
			ref Vector4 reference4 = ref baseVertices[num4];
			reference4 = new Vector4(vector4.x, vector4.y, vector4.z, num12);
		}
	}
	...
	{
		if (m_asyncUpdateTriggered && m_asyncUpdateSignal.WaitOne(100))
		{
			m_asyncUpdateTriggered = false;
		}
	}
